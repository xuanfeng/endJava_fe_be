...
function EndSkin(tmpId)
{
	...
	{	
		var file = path.normalize(RootPath+f);
		// var fileMatch = file.match(/views\/(.*?)\w/i);
		// file = file.replace(fileMatch[1],'');
		tempPath = file.replace(/\\[a-zA-Z_0-9]*\.jsp$/, "/"); //将此时的路径存起来
		console.log("+++++++++++++++++++++++++++++++++++++++" + file);
	...
	*/
	this.getTemplateString = function(id)
	{
		...
		*/
		...
		if (ms){
			var RootPathTemp = RootPath;  //这里先把默认的路径缓存，处理include时
				RootPath = tempPath;
...
//			console.log(ms);
		...
		if (ms = page.match(/\<c\:if.*\>/ig))//types:1 x>2;2 y > x;3 x;4 x==='test',support || or && like "a > 1 && b <10"
		{
//			console.log(ms);
			for(i=0; m=ms[i]; i++)
			{
